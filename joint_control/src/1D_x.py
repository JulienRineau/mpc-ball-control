...
def solve_cftoc(x0, u0):
    ...
    opti.subject_to(opti.bounded(-angle_acc_limit,rvel[0] - u0[0],angle_acc_limit))
...
def joint_vel(data, args):
    ...
    # timer.tic()
    pvel = solve_cftoc([x, velocity_x, current_position[0]], prev_angle_vel)
    # timer.toc('Solve Time Took: ')
    prev_angle_vel[0] = pvel

    dic_vel = {'right_j3':-pvel, 'right_j4':0}
